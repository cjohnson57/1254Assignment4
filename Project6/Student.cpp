...
double Student::calculateAdjustedGrade() const
{
	/* get homework portion */
	int lowestHw = homeWorks[0];
	for(int i = 1; i < NUM_OF_HOMEWORKS; i++)
		if(homeWorks[i] < lowestHw)
			lowestHw = homeWorks[i];
	double hw = hwWeight * (getTotalHW() - lowestHw) / (NUM_OF_HOMEWORKS - 1);
	
	/* get quiz portion */
	int lowestQuiz = quizzes[0];
	for(int i = 1; i < NUM_OF_QUIZZES; i++)
		if(quizzes[i] < lowestQuiz)
			lowestQuiz = quizzes[i];
	double q = quizWeight * (getTotalQuizzes() - lowestQuiz) / (NUM_OF_QUIZZES - 1);
...
string Student::outFileString() const
{
	return lastN + ", " + firstN + ":\t" + to_string(calculateGrade()) + "\t" + to_string(calculateAdjustedGrade());
} /* to string */
